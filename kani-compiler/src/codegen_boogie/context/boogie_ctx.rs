...
use boogie_ast::boogie_program::{BinaryOp, BoogieProgram, Expr, Function, Literal, Parameter, Procedure, Stmt, Type};
...
fn unsigned_less_than(width: usize) -> Function {
    Function::new(format!("$UnsignedLessThanBv{width}"),
                  vec![Parameter::new("lhs".into(), Type::Bv(width)),
...
fn unsigned_add(width: usize) -> Function {
    Function::new(format!("$UnsignedAddBv{width}"),
                  vec![Parameter::new("lhs".into(), Type::Bv(width)),
                       Parameter::new("rhs".into(), Type::Bv(width))],
                  Type::Bv(width),
                None,
...
impl<'a, 'tcx> FunctionCtx<'a, 'tcx> {
    pub fn new(bcx: &'a BoogieCtx<'tcx>, instance: Instance<'tcx>, mir: &'a Body<'tcx>) -> FunctionCtx<'a, 'tcx> {
    ...
    fn codegen_body(&self) -> Stmt {
        ...
        let statements: Vec<Stmt> = reverse_postorder(mir)
            .map(|(bb, bbd)| self.codegen_block(bb, bbd))
            .collect();
    ...
    fn codegen_funcall(
        ...
        match &funct.kind() {
            ty::FnDef(defid, substs) => {
                let instance =
                    Instance::expect_resolve(self.bcx.tcx, ty::ParamEnv::reveal_all(), *defid, substs);
