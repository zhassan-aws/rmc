...
use super::common::*;
use super::formatter::{DeclFormatter, FmtCtx, Formatter, IntoFormatter};
use super::gast::*;
use super::get_mir::MirLevel;
use super::llbc_ast;
use super::meta::{self, Attribute, ItemMeta, Span};
use super::meta::{FileId, FileName, InlineAttr, LocalFileId, Meta, VirtualFileId};
use super::names::Name;
use super::reorder_decls::{AnyTransId, DeclarationGroup, DeclarationsGroups, GDeclarationGroup};
use super::translate_predicates::NonLocalTraitClause;
use super::types::*;
use super::ullbc_ast as ast;
use super::values::*;
use hax_frontend_exporter as hax;
use hax_frontend_exporter::SInto;
use im::OrdMap;
use linked_hash_set::LinkedHashSet;
use charon_macros::VariantIndexArity;
...
macro_rules! error_or_panic {
    ($ctx:expr, $span:expr, $msg:expr) => {{
        $crate::codegen_aeneas_llbc::charon::translate_ctx::register_error_or_panic!($ctx, $span, $msg);
        let e = super::common::Error {
            span: $span,
            msg: $msg.to_string(),
        };
        return Err(e);
...
macro_rules! error_assert {
    ($ctx:expr, $span: expr, $b: expr) => {
        if !$b {
            ...
            $crate::codegen_aeneas_llbc::charon::translate_ctx::error_or_panic!($ctx, $span, msg);
        }
    };
...
impl<'tcx, 'ctx, 'ctx1> BodyTransCtx<'tcx, 'ctx, 'ctx1> {
    ...
    pub(crate) fn get_local_trait_clauses(&self) -> Vec<TraitClause> {
        let clauses: Vec<TraitClause> = self
            ...
            .collect();
        // Sanity check
        if !super::assumed::IGNORE_BUILTIN_MARKER_TRAITS {
            assert!(clauses
                .iter()
                .enumerate()
