...
fn type_to_string(ty: &Type) -> String {
    match ty {
        Type::Array(type_array) => {
            format!("[{}; {}]", type_to_string(&type_array.elem), expr_to_string(&type_array.len))
                .to_string()
        }
        Type::BareFn(_) => {
        ...
        Type::Tuple(type_tuple) => {
            let tys: Vec<String> = type_tuple.elems.iter().map(|ty| type_to_string(ty)).collect();
...
fn constraint_to_string(c: &Constraint) -> String {
    format!("{} : {}", c.ident.to_string(), type_param_bounds_to_string(&c.bounds)).to_string()
}
...
fn angle_bracketed_generic_arguments_to_string(
    ...
    if args.is_empty() { "".to_string() } else { format!("<{}>", args.join(", ")).to_string() }
}
...
fn path_segment_to_string(ps: &PathSegment) -> String {
    ...
    match &ps.arguments {
        ...
        PathArguments::AngleBracketed(args) => {
            format!("{}{}", seg, angle_bracketed_generic_arguments_to_string(&args.args))
                .to_string()
        }
...
fn path_to_string(path: &Path) -> String {
    let path: Vec<String> = path.segments.iter().map(|x| path_segment_to_string(x)).collect();
    path.join("::")
}
    ...
    match param {
        GenericParam::Type(type_param) => {
            ...
            if type_param.bounds.is_empty() || !with_constraints {
                ...
                format!("{} : {}", ident, type_param_bounds_to_string(&type_param.bounds))
                    .to_string()
            }
        }
        GenericParam::Lifetime(lf_param) => {
            ...
            if lf_param.bounds.is_empty() || !with_constraints {
                ...
                format!("{} : {}", ident, lifetime_bounds_to_string(&lf_param.bounds)).to_string()
            }
        }
...
fn generic_params_with_opt_constraints_to_string(
    ...
    let gens: Vec<String> = params
        ...
        .collect();
    if gens.is_empty() { "".to_string() } else { format!("<{}>", gens.join(", ")) }
}
    ...
    match wp {
        WherePredicate::Type(pred_type) => {
            ...
            if pred_type.bounds.is_empty() {
                ...
                format!("{} : {}", ty, type_param_bounds_to_string(&pred_type.bounds)).to_string()
            }
        }
        ...
        WherePredicate::Eq(pred_eq) => {
            format!("{} = {}", type_to_string(&pred_eq.lhs_ty), type_to_string(&pred_eq.rhs_ty))
                .to_string()
        }
    }
}
    ...
    let match_branches: Vec<String> = match &ast.data {
        Data::Enum(data) => {
            ...
            patterns
                ...
                .enumerate()
                .map(|(i, mp)| {
                    format!("{}{} => {{ ({}, {}) }},", THREE_TABS, mp.match_pattern, i, mp.num_args)
                        .to_string()
                })
                .collect()
        }
...
fn derive_enum_variant_method(item: TokenStream, method_kind: EnumMethodKind) -> TokenStream {
    ...
    let impls: Vec<String> = match &ast.data {
        ...
        Data::Struct(_) => {
            panic!("{} macro can not be called on structs", method_kind.variant_name());
        }
        Data::Union(_) => {
            panic!("{} macro can not be called on unions", method_kind.variant_name());
        }
    };
...
pub fn rust_version(_item: TokenStream) -> TokenStream {
    ...
    format!("\"+{}\"", toolchain.toolchain.channel).parse().unwrap()
}
...
fn generic_stmt_to_mut(s: &mut Stmt) {
    match s {
        Stmt::Local(s) => s.init.iter_mut().for_each(|e| generic_expr_to_mut(&mut e.1)),
...
fn generic_expr_to_mut(e: &mut Expr) {
    ...
    match e {
        ...
        Expr::If(e) => {
            ...
            e.else_branch.iter_mut().for_each(|(_, b)| generic_expr_to_mut(b));
        }
        Expr::Index(e) => {
            generic_expr_to_mut(&mut (*e.expr));
...
fn generic_mk_ident(id: &syn::Ident, to_mut: bool) -> syn::Ident {
    ...
    let id = id.to_string();
    let name = if to_mut { format!("Mut{}", id) } else { format!("Shared{}", id) };
...
pub fn make_generic_in_borrows(tokens: TokenStream) -> TokenStream {
    ...
    format!("{}\n{}", shared_item.to_token_stream(), mut_item.to_token_stream()).parse().unwrap()
}
