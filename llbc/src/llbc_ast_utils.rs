        ...
        C: AstFormatter,
    {
        match &self.content {
            ...
            RawStatement::Sequence(st1, st2) => {
                format!("{}\n{}", st1.fmt_with_ctx(tab, ctx), st2.fmt_with_ctx(tab, ctx))
            }
            RawStatement::Switch(switch) => match switch {
                ...
                Switch::SwitchInt(discr, _ty, maps, otherwise) => {
                    ...
                    format!("{}switch {} {{\n{}\n{}}}", tab, discr.fmt_with_ctx(ctx), maps, tab)
                }
            ...
            RawStatement::Loop(body) => {
                let inner_tab = format!("{tab}{TAB_INCR}");
                format!("{}loop {{\n{}\n{}}}", tab, body.fmt_with_ctx(&inner_tab, ctx), tab)
            }
        }
    }
}
