...
use crate::meta::Meta;
use crate::types::*;
pub use crate::ullbc_ast::{Call, FunDeclId, GlobalDeclId};
use crate::values::*;
...
pub enum Switch {
    ...
    /// }
    /// ```
    SwitchInt(Operand, IntegerTy, Vec<(Vec<ScalarValue>, Statement)>, Box<Statement>),
    /// A match over an ADT.
    ///
    ...
    /// switch into a match).
    Match(Place, Vec<(Vec<VariantId::Id>, Statement)>, Option<Box<Statement>>),
}
