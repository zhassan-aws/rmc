...
impl Terminator {
        ...
        C: AstFormatter,
    {
        match &self.content {
            ...
            RawTerminator::Assert { cond, expected, target } => {
                format!("assert({} == {}) -> bb{}", cond.fmt_with_ctx(ctx), expected, target)
            }
        }
    }
}
...
impl BlockData {
    ...
    pub fn transform_operands<F: FnMut(&Meta, &mut Vec<Statement>, &mut Operand)>(
        ...
        match &mut self.terminator.content {
            ...
            RawTerminator::Assert { cond, expected: _, target: _ } => {
                f(meta, &mut nst, cond);
            }
            RawTerminator::Panic
            ...
            | RawTerminator::Drop { place: _, target: _ } => {
                // Nothing to do
            }
        };
