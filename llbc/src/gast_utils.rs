...
pub fn make_locals_generator(locals: &mut VarId::Vector<Var>) -> impl FnMut(Ty) -> VarId::Id + '_ {
    move |ty| locals.push_with(|index| Var { index, name: None, ty })
}
        ...
        let items = {
            let items =
                self.parent_trait_refs
                    ...
                    .chain(self.required_methods.iter().chain(self.provided_methods.iter()).map(
                        |(name, f)| format!("{TAB_INCR}fn {name} = {}\n", ctx.format_object(*f)),
                    ))
                    .collect::<Vec<String>>();
            if items.is_empty() { "".to_string() } else { format!("\n{{\n{}}}", items.join("")) }
        };
...
impl<T> GFunDecl<T> {
        ...
        for<'a, 'b, 'c> <<C as SetGenerics<'a>>::C as SetLocals<'b>>::C: Formatter<&'c T>,
    {
        ...
        // Unsafe keyword
        let unsafe_kw =
            if self.signature.is_unsafe { "unsafe ".to_string() } else { "".to_string() };
