...
pub static IGNORED_TRAITS_NAMES: [&[&str]; 6] =
    [&MARKER_SIZED_NAME, &MARKER_TUPLE_NAME, &SYNC_NAME, &SEND_NAME, &UNPIN_NAME, &ALLOC_ALLOCATOR];
                ...
                if alloc == "alloc" && boxed == "boxed" && new == "new" {
                    match &impl_elem.kind {
                        ImplElemKind::Ty(Ty::Adt(TypeId::Assumed(AssumedTy::Box), generics)) => {
                            let GenericArgs { regions, types, const_generics, trait_refs } =
                                generics;
                            if regions.is_empty()
                                && types.len() == 1
...
pub fn function_to_info(name: &Name) -> Option<FunInfo> {
    ...
    match get_fun_id_from_name_full(name) {
        ...
        Option::Some(id) => {
            let info = match id {
                FunId::Panic => FunInfo { used_type_params: vec![], used_args: vec![true] },
                FunId::BeginPanic => {
                    FunInfo { used_type_params: vec![true], used_args: vec![true] }
                }
                FunId::BoxNew => FunInfo { used_type_params: vec![true], used_args: vec![true] },
                FunId::BoxFree => {
                    FunInfo { used_type_params: vec![true, false], used_args: vec![true, false] }
                }
            };
            Option::Some(info)
        }
