...
impl GenericParams {
    pub fn len(&self) -> usize {
        let GenericParams { regions, types, const_generics, trait_clauses } = self;
...
pub fn fmt_where_clauses(tab: &str, num_parent_clauses: usize, clauses: Vec<String>) -> String {
    if clauses.is_empty() {
        ...
        let mut clauses =
            clauses.iter().map(|x| format!("\n{tab}{TAB_INCR}{x},")).collect::<Vec<String>>();
...
impl Predicates {
    pub fn is_empty(&self) -> bool {
        let Predicates { regions_outlive, types_outlive, trait_type_constraints } = self;
    ...
    C: AstFormatter,
{
    ...
    let mut regions_outlive: Vec<_> = preds
        ...
        .map(|OutlivesPred(x, y)| format!("{} : {}", x.fmt_with_ctx(ctx), y.fmt_with_ctx(ctx)))
        .collect();
    let mut type_constraints: Vec<_> =
        preds.trait_type_constraints.iter().map(|x| x.fmt_with_ctx(ctx)).collect();
    match info {
        None => {
        ...
        Some(info) => {
            ...
            let local_clauses: Vec<_> = trait_clauses
                ...
                .chain(type_constraints.split_off(info.num_trait_type_constraints).into_iter())
                .collect();
            ...
            let all_clauses: Vec<_> =
                inherited_clauses.into_iter().chain(local_clauses.into_iter()).collect();
...
impl GenericArgs {
    pub fn len(&self) -> usize {
        let GenericArgs { regions, types, const_generics, trait_refs } = self;
    ...
    pub fn new(
        ...
        GenericArgs { regions, types, const_generics, trait_refs }
    }
        ...
        C: AstFormatter,
    {
        ...
        let clauses =
            if clauses.is_empty() { "".to_string() } else { format!("[{}]", clauses.join(", ")) };
        ...
        C: AstFormatter,
    {
        match self {
            ...
            TraitInstanceId::Closure(fid, generics) => {
                format!("(closure:{}{})", ctx.format_object(*fid), generics.fmt_with_ctx(ctx))
            }
            TraitInstanceId::Unsolved(trait_id, generics) => {
                format!("Unsolved({}{})", ctx.format_object(*trait_id), generics.fmt_with_ctx(ctx),)
            }
        ...
        match &self.kind {
            TypeDeclKind::Struct(fields) => {
                if !fields.is_empty() {
                    ...
                    format!("struct {}{params}{preds}{eq_space}= {{}}", self.name.fmt_with_ctx(ctx))
                }
            }
            TypeDeclKind::Enum(variants) => {
                let variants: Vec<String> =
                    variants.iter().map(|v| format!("|  {}", v.fmt_with_ctx(ctx))).collect();
            ...
            TypeDeclKind::Error(msg) => {
                format!("opaque type {}{params}{preds} = ERROR({msg})", self.name.fmt_with_ctx(ctx),)
            }
        }
    }
}
        ...
        match self {
            ...
            Ty::Arrow(regions, inputs, box output) => {
                ...
                let regions = if regions.is_empty() {
                    ...
                    format!(
                        "<{}>",
                        regions.iter().map(|r| r.to_string()).collect::<Vec<String>>().join(", ")
                    )
                };
                let inputs =
                    inputs.iter().map(|x| x.fmt_with_ctx(ctx)).collect::<Vec<String>>().join(", ");
                if output.is_unit() {
...
impl FunSig {
        ...
        C: AstFormatter,
    {
        ...
        // Unsafe keyword
        let unsafe_kw = if self.is_unsafe { "unsafe ".to_string() } else { "".to_string() };
